...
export function HeadingText({
  ...
  return (
    <>
      {headingLevel === 'h1' && (
        <h1 className={getHeadingClassName(level, weight, className)}>{children}</h1>
      )}
      {headingLevel === 'h2' && (
        <h2 className={getHeadingClassName(level, weight, className)}>{children}</h2>
      )}
      {headingLevel === 'h3' && (
        <h3 className={getHeadingClassName(level, weight, className)}>{children}</h3>
      )}
      {headingLevel === 'h4' && (
        <h4 className={getHeadingClassName(level, weight, className)}>{children}</h4>
      )}
      {headingLevel === 'h5' && (
        <h5 className={getHeadingClassName(level, weight, className)}>{children}</h5>
      )}
      {headingLevel === 'h6' && (
        <h6 className={getHeadingClassName(level, weight, className)}>{children}</h6>
      )}
    </>
  );
}
...
export function BodyText({ children, level, weight, className }: IBodyTextProps) {
  return <p className={getBodyClassName(level, weight, className)}>{children}</p>;
}
...
export function Label({ children, size, weight, className, htmlFor = '', ...props }: ILabelProps) {
  return (
    <label className={getLabelClassName(size, weight, className)} htmlFor={htmlFor} {...props}>
      {children}
    </label>
  );
}
