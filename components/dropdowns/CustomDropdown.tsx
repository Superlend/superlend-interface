        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button size="md" variant="outline" className="group flex items-center gap-1 data-[state=open]:ring-2 data-[state=open]:ring-secondary-500 text-gray-800 rounded-2 focus-visible:ring-0">
                    <ImageWithDefault src={selectedOption?.token.logo} alt={selectedOption?.token.symbol} width={18} height={18} className='rounded-full max-w-[18px] max-h-[18px]' />
                    <BodyText level='body2' weight='medium' className="text-gray-800">{selectedOption?.token.symbol}</BodyText>
            ...
            <DropdownMenuContent className="p-0 rounded-[16px] border-none bg-white bg-opacity-40 backdrop-blur-md overflow-hidden">
                <DropdownMenuLabel className="text-gray-600 py-2 px-4">Select Option</DropdownMenuLabel>
                {
                    options?.map((option: any) => (
                        ...
                        >
                            <ImageWithDefault src={option?.token?.logo || ""} alt={option?.token?.symbol || ""} width={20} height={20} className='rounded-full max-w-[20px] max-h-[20px]' />
                            <BodyText level='body2' weight='medium' className="text-gray-800">{option?.token?.symbol || ""}</BodyText>
                        </DropdownMenuItem>
                    ))
                }
            </DropdownMenuContent>
