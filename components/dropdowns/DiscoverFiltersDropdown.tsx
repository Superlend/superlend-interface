...
export default function DiscoverFiltersDropdown() {
  ...
  const [isStablecoinsSelected, setIsStablecoinsSelected] = useState(
    STABLECOINS_NAMES_LIST.every((name) => filters.token_ids.includes(name)) || false
  );
  const { width: screenWidth } = useDimensions();
  const isDesktop = screenWidth > 768;

  const hasActiveFilters =
    !!filters.token_ids.length || !!filters.chain_ids.length || !!filters.protocol_ids.length;
  const activeFiltersTotalCount =
    filters.token_ids.length + filters.chain_ids.length + filters.protocol_ids.length;
  const getActiveFiltersCountByCategory = (filterName: keyof typeof filters) =>
  ...
  const allTokenOptions = Object.values(allTokensData)
    ...
    .reduce((acc: TToken[], current: any) => {
      if (
        !acc.some((item) => item.symbol.toLocaleUpperCase() === current.symbol.toLocaleUpperCase())
      ) {
        acc.push(current);
      }
      return acc;
  ...
  const selectStablecoins = () => {
    if (!isStablecoinsSelected) {
      ...
      STABLECOINS_NAMES_LIST.forEach((el) => {
        const findToken = allTokenOptions.find((item: any) => item?.token_id === el);
    ...
    updateSearchParams({
      token_ids: currentTokenIds.length ? currentTokenIds.join(',') : undefined,
    });
  };
...
function FilterCardContent({
  ...
  const isActiveTab = (item: any) =>
    activeTab.value === item.value ? 'text-black bg-gray-300' : 'text-gray-500 hover:bg-gray-200';
...
function FilterOptions({
  ...
  const handleSelection = (id: number | string | null, filterType: string) => {
    ...
    updateSearchParams({
      [`${filterType}_ids`]: newFilters.length ? newFilters.join(',') : undefined,
    });
  };
  ...
  return (
    <ScrollArea id="filter-options-section" className="h-[400px] sm:h-[250px] w-full pt-5">
      ...
      {type === 'token' && (
        <Button
          variant={isStablecoinsSelected ? 'secondaryOutline' : 'outline'}
