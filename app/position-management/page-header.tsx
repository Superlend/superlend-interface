...
export default function PageHeader() {
  ...
  useEffect(() => {
    const hasNoData =
      isErrorPlatformData || (!tokenDetails?.symbol?.length && !chainDetails?.name?.length);
    if (hasNoData && !isLoadingPlatformData) {
      return notFound();
    }
  }, [isErrorPlatformData, tokenAddress, chain_id, protocol_identifier, isLoadingPlatformData]);
  ...
  const isFluidVault =
    platformData?.platform?.protocol_type === PlatformType.FLUID && platformData?.platform?.isVault;
  ...
  const checkForPairBasedTokens = (platformTypes: string[], platformType: string) =>
    platformTypes.map((type) => type?.toLowerCase()).includes(platformType?.toLowerCase());
  ...
  const getCollateralTokenDetails = (tokenSymbol: string) => {
    const collateralTokenDetails = allTokensData[Number(chain_id)]?.find(
      (token: any) => token?.symbol?.toLowerCase() === tokenSymbol?.toLowerCase()
    );
  ...
  const hasWarnings =
    platformData.assets.filter((asset: TPlatformAsset) => asset?.token?.warnings?.length > 0)
      .length > 0;
      ...
      {hasWarnings && (
        <div className="flex flex-col gap-4">
          {warningMessages.map((message: any, index: number) => (
            <AlertWarning
              key={index}
              description={WarningMessages[message.type as keyof typeof WarningMessages]}
            />
          ))}
        </div>
      )}
              ...
              <div className="flex items-center flex-wrap gap-[12px]">
                ...
                {isDisplayOneToken && (
                  <div className="one-token flex items-center gap-[8px]">
                    ...
                    />
                    <HeadingText level="h4" weight="medium" className="break-words text-gray-800">
                      {tokenSymbol}
                    </HeadingText>
                  </div>
                )}
              ...
              {!isLoadingPlatformData && platformLogo && (
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <Label weight="medium" className="leading-[0] shrink-0 capitalize">
                  ...
                  >
                    <span className="uppercase text-secondary-500 font-medium">
                      {platformId.split('-')[0]} {getPlatformVersion(platformId)}
                    </span>
        ...
        description={
          <div className="flex flex-col gap-[4px]">
            <BodyText level="body2" weight="normal">
              ...
              <span className="mx-2">
                Supplying directly to Morpho markets is risky. It requires knowledge, and the right
                tools to manage risks properly, and there is a chance that your deposit may become
                illiquid. More details can be found
              </span>
              <a
