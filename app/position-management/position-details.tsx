...
export default function PositionDetails() {
    ...
    for (const platform of userPositions) {
      for (const position of platform.positions) {
        if (position.type === 'lend') {
          borrowPower +=
            ((position.amount * position.liquidation_threshold) / 100) * position.token.price_usd;
  ...
  function getLiquidationDetailsForPairBasedAssets() {
    ...
    const denominator =
      Number(formattedUserPositions?.lendAsset?.tokenDetails[0]?.liquidation_threshold) / 100;
    ...
    const liquidationPercentage =
      (Number(formattedUserPositions?.borrowAsset?.tokenDetails[0]?.amount) * 100) /
      (Number(formattedUserPositions?.lendAsset?.tokenDetails[0]?.amount) * denominator);
        ...
        <div className="px-[16px]">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 mb-[12px]">
            <div className="flex items-center gap-[8px]">
              ...
              {liquidationDetails.hasBorrowed && (
                <Badge
                  variant={
                    liquidationDetails.riskFactor.theme as 'destructive' | 'yellow' | 'green'
                  }
                >
              ...
              {!liquidationDetails.hasBorrowed && <Badge variant="default">N/A</Badge>}
            </div>
              ...
              <div className="flex items-center gap-[6px]">
                ...
                {liquidationDetails.hasBorrowed && liquidationDetails.liquidationPrice !== 0 && (
                  <BodyText level="body1" weight="medium">
                    ${abbreviateNumber(liquidationDetails.liquidationPrice)}
                  </BodyText>
                )}
                {(!liquidationDetails.hasBorrowed || liquidationDetails.liquidationPrice === 0) && (
                  <BodyText level="body1" weight="normal">
                    <InfoTooltip
                      label={<TooltipText className="text-gray-600">N/A</TooltipText>}
          ...
          <div className="progress-bar mb-[20px]">
            <Progress
              ...
              variant={liquidationDetails.riskFactor.theme as 'destructive' | 'yellow' | 'green'}
            />
          </div>
        </div>
      )}
