...
export function EstimatedReturns({ platformDetails }: { platformDetails: TPlatform }) {
      ...
      if (positionType === 'lend') {
        ...
        setLendAssetDetails(
          platformDetails?.assets.find(
            (asset) => asset.token.address.toLowerCase() === tokenAddress.toLowerCase()
          )
        );
  ...
  const lendTokenDetails =
    isAaveV3 && positionType === 'borrow' ? selectedStableTokenDetails : lendAssetDetails;
  const borrowTokenDetails =
    isAaveV3 && positionType === 'lend' ? selectedStableTokenDetails : borrowAssetDetails;
  ...
  useEffect(() => {
    setSelectedStableTokenDetails(
      positionType === 'borrow' ? stableLendAssetsList[0] : stableBorrowAssetsList[0]
    );
  ...
  const rows: TRow[] = [
    ...
    },
    {
      ...
      totalValue:
        (isMorpho && positionType === 'lend') || (!lendAssetDetails && isCompoundV2)
          ? 25000
          : isUSDAmount
            ? maxBorrowAmountInUsd
            : maxBorrowAmountInUsd / (borrowTokenDetails?.token.price_usd ?? 0),
  ...
  const { interestGain, interestLoss, netEstimatedEarnings } = getEstimatedEarnings({
    supplyAPY,
    borrowAPY,
    amountSupplied,
    amountBorrowed,
  ...
  const netEstimatedEarningFinal = isUSDAmount
    ...
    : interestGain * (lendAssetDetails?.token?.price_usd ?? 0) -
      interestLoss * (borrowAssetDetails?.token?.price_usd ?? 0);
              ...
              <InfoTooltip
                label={
                  <HeadingText level="h4" weight="medium" className="text-gray-800">
                    <TooltipText>
                      {containsNegativeInteger(netEstimatedEarningFinal) ? '-' : ''}$
                      {abbreviateNumber(
                        Number(convertNegativeToPositive(netEstimatedEarningFinal))
                      )}
                    </TooltipText>
                  </HeadingText>
                }
                  ...
                  !isMorpho ? (
                    <div className="flex flex-col gap-[12px]">
                      <div className="flex flex-col gap-[4px]">
                        ...
                        <div className="flex items-center gap-[6px]">
                          <HeadingText level="h5" weight="medium" className="text-gray-800">
                            {containsNegativeInteger(interestGain) ? '-' : ''}
                            {isUSDAmount ? '$' : ''}
                            {abbreviateNumber(Number(convertNegativeToPositive(interestGain)))}
                          </HeadingText>
                          <ImageWithDefault
                            src={lendTokenDetails?.token.logo}
                            alt={lendTokenDetails?.token.symbol}
                            width={20}
                            height={20}
                      ...
                      <div className="flex flex-col gap-[4px]">
                        ...
                        <div className="flex items-center gap-[6px]">
                          <HeadingText level="h5" weight="medium" className="text-gray-800">
                            {containsNegativeInteger(interestLoss) ? '-' : ''}
                            {isUSDAmount ? '$' : ''}
                            {abbreviateNumber(Number(convertNegativeToPositive(interestLoss)))}
                          </HeadingText>
                          <ImageWithDefault
                            src={borrowTokenDetails?.token.logo}
                            alt={borrowTokenDetails?.token.symbol}
                            width={20}
                            height={20}
                            className="rounded-full max-w-[20px] max-h-[20px]"
                          />
            ...
            {rows
              ...
              .map((row) => (
                <div key={row.id} className="flex flex-col gap-[16px]">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-[8px]">
                      ...
                      <div className="flex items-center gap-[6px]">
                        {!isAssetNotAvailable(row) && (
                          <>
                            <HeadingText level="h5" weight="medium" className="text-gray-800">
                              {getDisplayedValuePrefix(row.key)}
                              {abbreviateNumber(row.selectedValue, row.key === 'duration' ? 0 : 2)}
                            </HeadingText>
                            ...
                            {!row.hasSelectedValue && (
                              <StableTokensDropdown
                                options={
                                  row.key === 'lend' ? stableLendAssetsList : stableBorrowAssetsList
                                }
                    ...
                    {!isAssetNotAvailable(row) && (
                      <div className="flex items-center gap-[6px]">
                        <BodyText level="body1" weight="normal" className="text-gray-600">
                          {getDisplayedValuePrefix(row.key)}
                          {abbreviateNumber(
                            row.key === 'duration' ? row.totalValue / 12 : row.totalValue,
                            row.key === 'duration' ? 0 : 2
                          )}
                          {getDisplayedValueSufix(row.key)}
                        </BodyText>
                        {row.logo && (
                          <ImageWithDefault
                            src={row.logo}
                  ...
                  <Slider
                    disabled={
                      isAssetNotAvailable(row) || (row.key === 'borrow' && row.totalValue === 0)
                    }
          ...
          {options?.map((asset: any) => (
            <DropdownMenuItem
              ...
              className={cn(
                ...
                selectedStableTokenDetails?.token?.address === asset?.token?.address &&
                  'bg-gray-400'
              )}
            >
              <ImageWithDefault
