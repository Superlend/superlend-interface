...
export default function PortfolioOverview() {
  ...
  const NET_WORTH = getStatDisplayValue(
    Number(portfolioData?.total_supplied ?? 0) - Number(portfolioData?.total_borrowed ?? 0)
  );
  const EARNINGS = getStatDisplayValue(
    Number(portfolioData?.platforms.reduce((acc, curr) => acc + curr.pnl, 0) ?? 0)
  );
  ...
  return (
    ...
    >
          ...
          <div className="positions-net-worth-block h-full px-[24px] md:px-[32px] pt-[28px] flex flex-col items-start justify-between gap-[29px] pb-[24px]">
            <div className="shrink-0">
              ...
              {!isLoadingPortfolioData && (
                <HeadingText level="h2" weight="medium" className="text-gray-800">
                  {NET_WORTH}
                </HeadingText>
              )}
                  ...
                  <React.Fragment key={positionIndex}>
                    <div className="data-block-1">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <Label className="text-gray-600 text-success-500 capitalize">
                          Your {position.label}
                        </Label>
                      </div>
                    </div>
      ...
      <article className="h-full">
        <UserPositionsByPlatform data={portfolioData} isLoading={isLoadingPortfolioData} />
      </article>
    </section>
  );
}
