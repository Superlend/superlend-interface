...
export default function AllPositions() {
  ...
  const filteredTableData = rawTableData.filter((position) => {
    const matchesTokenFilter =
      filters.token_ids.length === 0 || filters.token_ids.includes(position.tokenSymbol);
    const matchesPlatformFilter =
      filters.platform_ids.length === 0 || filters.platform_ids.includes(position.platformName);
    const matchesChainFilter =
      filters.chain_ids.length === 0 ||
      filters.chain_ids.map((chain) => chain.toString()).includes(position.chain_id.toString());
  ...
  return (
    <section id="all-positions" className="all-positions-container flex flex-col gap-[24px] px-5">
        ...
        <div className="all-positions-header-left w-full lg:w-auto flex flex-col lg:flex-row items-start lg:items-center gap-[20px] lg:gap-[12px]">
          <div className="flex items-center justify-between gap-[12px] max-lg:w-full">
            <div className="flex items-center gap-[12px]">
              <HeadingText level="h3" weight="medium" className="capitalize text-gray-800">
                All positions
              </HeadingText>
              <InfoTooltip
                content={
                  <div className="flex flex-col gap-[4px]">
                    <BodyText level="body3">
                      Track all your lending and borrowing positions in one place.
                    </BodyText>
                  </div>
                }
              />
      ...
      <div className="all-positions-content">
        ...
        {isLoadingPortfolioData && <LoadingSectionSkeleton className="h-[300px] md:h-[400px]" />}
      </div>
    </section>
  );
}
