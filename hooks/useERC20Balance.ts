...
export const useERC20Balance = (address: string) => {
  ...
  const chainList: string[] = allChainsData.map((chain) => chain.chain_id.toString());
  const [data, setData] = useState<
    Record<number, Record<string, { balanceRaw: string; balanceFormatted: number }>>
  >({});
  ...
  useEffect(() => {
    if (!!address && !!tokenList && chainList.length > 0 && Object.keys(data).length === 0) {
