...
import { getLoopOpportunitiesData } from '@/queries/loop-opportunities-api'
...
// └─────────────────────────────────────────────────────────────┘`, responseData)
    ...
    const dataToReturn = data || (hasFreshCache() && cachedData.length > 0 ? cachedData : [])

    return { 
        data: dataToReturn, 
        isLoading: isLoading && !data && dataToReturn.length === 0,
        isError, 
